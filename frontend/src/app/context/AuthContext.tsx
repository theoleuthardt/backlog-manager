...
/**
 * `sessionGeneration` is bumped on every login()/logout()/token-cleared
 * event so a getCurrentUser() call that was already in flight when one
 * of those happened can tell its result is stale and skip applying it -
 * otherwise a slow request started before a logout (or before a second
 * login) could resolve afterwards and silently resurrect the old
 * session.
 */
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  /**
   * A catch here means the stored token is missing/expired/invalid - the
   * apiClient's response middleware already cleared it on a 401.
   */
